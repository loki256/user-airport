...
object GeoHashMap extends LazyLogging {
  ...
  @tailrec
  def getClosestAirportForUser(user: User, geohashSize: Int, getAirportsByGeohash: (String => Seq[AirPort])): Option[AirPort] = {
    if (geohashSize == 0) {
      None
    } else {
      val userGeoHash: String = GeoHash.geoHashStringWithCharacterPrecision(user.lat, user.lon, geohashSize)
      val ports = getAirportsByGeohash(userGeoHash)
      if (ports.nonEmpty) {
  ...
  def calculate(usersIterator: Iterator[User], airportsIterator: Iterator[AirPort]): Iterator[UserAirportResult] = {
    ...
    val airportRetriever = (userGeohashStr: String) => {
      val geohashAccuracy: Int = userGeohashStr.length
      ...
      mp.getOrElse(userGeohashStr, Seq())
    }
